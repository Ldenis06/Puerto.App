...
export const NameLoginGate: React.FC<Props> = ({ users, onLogin }) => {
  ...
  return <div className="fixed inset-0 z-[60] overflow-y-auto bg-[radial-gradient(circle_at_50%_20%,rgba(10,132,255,0.22),transparent_32%),#000] px-4 py-10">
    <div className="mx-auto flex min-h-full max-w-md items-center justify-center"><section className="w-full rounded-[30px] border border-white/15 bg-zinc-950/95 p-6 shadow-[0_20px_70px_rgba(0,0,0,0.8)]">
      ...
      <p className="mt-5 flex items-start gap-2 rounded-xl border border-amber-400/20 bg-amber-400/5 p-3 text-[11px] leading-relaxed text-zinc-400"><User className="mt-0.5 h-4 w-4 shrink-0 text-amber-300" />Cada integrante ingresa con la contraseña de su propio perfil.</p>
    </section></div>
    {protectedMember && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm"><form onSubmit={submitProfile} className="w-full max-w-sm rounded-[26px] border border-[#0A84FF]/35 bg-zinc-950 p-5 shadow-2xl"><div className="mb-4 flex items-center gap-3"><img src={protectedMember.avatarUrl} alt="" className="h-12 w-12 rounded-full border border-[#0A84FF]/50" /><div><h2 className="font-black text-white">Acceso de {protectedMember.name}</h2><p className="text-xs text-zinc-400">{protectedMember.id === 'denis' ? 'Ingresá la contraseña para habilitar administración.' : 'Ingresá la contraseña de este perfil.'}</p></div></div><label className="mb-1.5 block text-xs font-semibold text-zinc-300" htmlFor="profile-password">Contraseña</label><div className="relative"><KeyRound className="absolute left-3 top-3 h-4 w-4 text-zinc-500" /><input id="profile-password" type="password" autoFocus autoComplete="current-password" value={password} onChange={(event) => { setPassword(event.target.value); setError(null); }} className="w-full rounded-xl border border-white/20 bg-black py-2.5 pl-10 pr-3 text-sm text-white outline-none focus:border-[#0A84FF]" /></div>{error && <p role="alert" className="mt-2 text-xs text-red-300">{error}</p>}<div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => setProtectedMember(null)} className="rounded-xl border border-white/15 py-2.5 text-xs font-bold text-zinc-300">Cancelar</button><button type="submit" className="rounded-xl bg-[#0A84FF] py-2.5 text-xs font-bold text-white">Entrar</button></div></form></div>}
  </div>;
};
