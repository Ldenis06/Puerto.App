...
interface Props {
  ...
  onRequestGoogleLogin: () => Promise<string>;
}

export const GoogleLoginGate: React.FC<Props> = ({ users, onLogin, onLinkGoogleAuth, onRequestGoogleLogin }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    setErrorMsg('');
    try {
      handleProcessGoogleEmail(await onRequestGoogleLogin());
    } catch (error) {
      setErrorMsg(error instanceof Error ? error.message : 'No se pudo iniciar sesión con Google.');
    } finally {
