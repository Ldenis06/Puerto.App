...
import { BookOpenText, Check, Clipboard, LoaderCircle, Send } from 'lucide-react';
import { NotebookNote, NotebookReaction } from '../../types';
import { getNotebookNotes, getNotebookReactions, getNotebookSessionUserId, publishNotebookNote, toggleNotebookReaction } from '../../services/supabase';
...
export const LibretasTab: React.FC<Props> = ({ isDenis }) => {
  ...
  const copyTemplate = async () => {
    try {
      await navigator.clipboard.writeText(PROFILE_TEMPLATE);
      setCopied(true);
      window.setTimeout(() => setCopied(false), 1800);
    } catch { setError('No se pudo copiar automáticamente. Mantené presionado el texto para copiarlo.'); }
  };
  ...
  return <div className="space-y-4 pb-24 animate-fadeIn">
    ...
    {!isDenis && <section className="rounded-[26px] border border-white/10 bg-white/[0.04] p-4"><div className="mb-2 flex items-center justify-between gap-3"><h3 className="text-xs font-bold text-white">Plantilla de usuario</h3><button type="button" onClick={() => void copyTemplate()} className="flex items-center gap-1.5 rounded-xl border border-[#0A84FF]/35 bg-[#0A84FF]/15 px-3 py-1.5 text-[11px] font-bold text-[#5AC8FA]">{copied ? <><Check className="h-3.5 w-3.5" /> Copiado</> : <><Clipboard className="h-3.5 w-3.5" /> Copiar</>}</button></div><pre className="select-text whitespace-pre-wrap rounded-xl border border-white/10 bg-black/40 p-3 text-xs leading-relaxed text-zinc-300">{PROFILE_TEMPLATE}</pre></section>}

    {error && <div role="alert" className="rounded-2xl border border-red-500/30 bg-red-500/10 p-3 text-xs text-red-200">{error}</div>}
    {loading ? <div className="flex justify-center py-10 text-zinc-400"><LoaderCircle className="h-6 w-6 animate-spin" /></div> : notes.length === 0 ? <div className="rounded-[26px] border border-white/10 bg-white/[0.03] p-8 text-center"><BookOpenText className="mx-auto mb-3 h-9 w-9 text-zinc-500" /><p className="text-sm font-bold text-white">Todavía no hay publicaciones</p></div> : <div className="space-y-3">{notes.map((note) => { const noteReactions = reactions.filter((reaction) => reaction.note_id === note.id); return <article key={note.id} className="rounded-[24px] border border-white/10 bg-white/[0.04] p-4"><div className="mb-2 flex items-center justify-between gap-3"><span className="text-xs font-bold text-[#5AC8FA]">Sistema</span><time className="text-[10px] text-zinc-500">{new Date(note.created_at).toLocaleString('es-AR', { dateStyle: 'short', timeStyle: 'short' })}</time></div><p className="whitespace-pre-wrap text-sm leading-relaxed text-zinc-100">{note.body}</p><div className="mt-3 flex flex-wrap gap-1.5 border-t border-white/10 pt-3">{emojis.map((emoji) => { const count = noteReactions.filter((reaction) => reaction.emoji === emoji).length; const active = noteReactions.some((reaction) => reaction.emoji === emoji && reaction.user_id === sessionUserId); return <button key={emoji} type="button" onClick={() => void react(note.id, emoji)} className={`rounded-xl border px-2.5 py-1 text-xs transition ${active ? 'border-[#0A84FF]/70 bg-[#0A84FF]/20 text-white' : 'border-white/10 bg-black/30 text-zinc-300 hover:bg-white/10'}`}>{emoji}{count > 0 && <span className="ml-1">{count}</span>}</button>; })}</div></article>; })}</div>}
  </div>;
};
