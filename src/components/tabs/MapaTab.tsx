...
interface Props {
  ...
  onSetPresetScenario: (scenario: 'couple' | 'group' | 'all' | 'dispersed') => void;
}
...
export const MapaTab: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    // Handle map click to place/move location manually
    map.on('click', async (e: L.LeafletMouseEvent) => {
      const { lat, lng } = e.latlng;
      const label = await reverseGeocode(lat, lng);
      setLocationSource('manual');
      setGpsStatusInfo(`Ubicación manual fijada: ${label}`);
      setGpsError(null);
      if (currentUser) {
        onUpdateUserLocation(currentUser.id, lat, lng, true, label);
      }
    });
    ...
    // Auto-request real GPS if not yet active or was previously active
    if (!currentUser?.location?.isActive) {
      handleRequestCellPhoneGps(false);
    }
  ...
  const handleRequestCellPhoneGps = async (showErrors = true) => {
    ...
    setIsLocating(true);
    setGpsError(null);
    setGpsStatusInfo('Solicitando ubicación GPS a tu celular... Presiona "Permitir" si aparece la ventana');
  ...
  // Simulate bringing Maxi right next to user's real phone location (20m) to test the love relationship alert here!
  const handleBringMaxiHere = () => {
    if (!currentUser?.location?.isActive) {
      handleRequestCellPhoneGps(true);
      return;
    }

    const myLat = currentUser.location.lat;
    const myLng = currentUser.location.lng;

    // Place Maxi ~20 meters away
    const maxiLat = myLat + 0.00018;
    const maxiLng = myLng + 0.00018;

    const maxiUser = users.find((u) => u.id === 'maxi') || users.find((u) => u.id !== currentUser.id);
    if (maxiUser) {
      onUpdateUserLocation(
        maxiUser.id,
        maxiLat,
        maxiLng,
        true,
        `Al lado de ${currentUser.name}`
      );
      if (mapInstanceRef.current) {
        mapInstanceRef.current.flyTo([myLat, myLng], 18, { duration: 1 });
      }
    }
  };
  ...
  return (
    <div className="space-y-4 pb-24 animate-fadeIn">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-300 mt-0.5">
                {currentUser?.location?.isActive
                  ...
                  : 'Presiona el botón para localizar tu teléfono donde sea que estés'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              ...
              <span>{isLocating ? 'Detectando GPS...' : '📍 Ubicar mi Celular Aquí'}</span>
            </button>
      ...
      {/* Love Alert Tester at User's Current Location */}
      <div className="p-4 rounded-[26px] bg-white/[0.04] border border-white/10 backdrop-blur-xl">
        <div className="flex items-center justify-between mb-2.5">
          <div className="text-xs font-bold text-zinc-300 uppercase tracking-wider flex items-center gap-1.5">
            ...
            <span>Probar Alertas de Proximidad en Tu Ubicación</span>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {/* Button to bring Maxi right next to user's real phone location */}
          <button
            type="button"
            onClick={handleBringMaxiHere}
            className="p-3 rounded-2xl bg-gradient-to-r from-pink-500/20 to-rose-500/20 border border-pink-500/40 text-left transition active:scale-95 group hover:border-pink-500"
          >
            <div className="text-xs font-extrabold text-pink-300 flex items-center gap-1.5">
              <Heart className="w-4 h-4 fill-pink-400 text-pink-400 group-hover:scale-110 transition" />
              <span>Traer a Maxi a mi lado (≤ 20m)</span>
            </div>
            <div className="text-[11px] text-zinc-300 mt-1">
              Coloca a Maxi al lado de tu celular para activar aquí mismo la alerta: <em>"están teniendo relaciones amorosas"</em>.
            </div>
          </button>
          ...
          <div className="grid grid-cols-3 gap-1.5">
            <button
              type="button"
              onClick={() => onSetPresetScenario('couple')}
              className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-center transition active:scale-95"
            >
              <div className="text-xs font-bold text-pink-400">2 Miembros</div>
              <div className="text-[9px] text-zinc-400 mt-0.5">Amorosa</div>
            </button>

            <button
              type="button"
              onClick={() => onSetPresetScenario('group')}
              className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-center transition active:scale-95"
            >
              <div className="text-xs font-bold text-amber-400">4 Miembros</div>
              <div className="text-[9px] text-zinc-400 mt-0.5">Juntos</div>
            </button>

            <button
              type="button"
              onClick={() => onSetPresetScenario('dispersed')}
              className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-center transition active:scale-95"
            >
              <div className="text-xs font-bold text-zinc-300">Separados</div>
              <div className="text-[9px] text-zinc-400 mt-0.5">&gt; 50m</div>
            </button>
          </div>
        </div>
      </div>
