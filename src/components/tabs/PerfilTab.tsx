...
interface Props { currentUser: User | null; users: User[]; onUpdateUser: (updatedUser: User) => void; onUpdateAvatar: (avatarUrl: string, password?: string) => Promise<void>; onUpdateAiDescription: (userId: string, description: string, password: string) => Promise<void>; isAdmin: boolean; onClearAllExpenses: () => void; canChangeProfile: boolean; onUseProfileChange: () => void; }

export const PerfilTab: React.FC<Props> = ({ currentUser, users, onUpdateUser, onUpdateAvatar, onUpdateAiDescription, isAdmin, onClearAllExpenses, canChangeProfile, onUseProfileChange }) => {
  ...
  const [selectedDescriptionUserId, setSelectedDescriptionUserId] = useState('');
  const [managedDescription, setManagedDescription] = useState('');
  const [descriptionSaving, setDescriptionSaving] = useState(false);
  const [descriptionError, setDescriptionError] = useState<string | null>(null);
  const selectedDescriptionUser = users.find((member) => member.id === selectedDescriptionUserId) || users[0] || null;
  useEffect(() => {
    if (selectedDescriptionUser) setManagedDescription(selectedDescriptionUser.aiDescription);
  }, [selectedDescriptionUser?.id, selectedDescriptionUser?.aiDescription]);
  ...
  const updateAvatar = (file: File) => {
    ...
    reader.onload = (event) => {
      ...
      image.onload = async () => {
        const ratio = Math.min(192 / image.width, 192 / image.height, 1);
        ...
        const avatarUrl = canvas.toDataURL('image/jpeg', 0.72);
        const password = isAdmin ? window.prompt('Ingresá la contraseña de Denis para sincronizar la foto.') || undefined : undefined;
        if (isAdmin && !password) return;
        try {
          setAvatarSaving(true);
          await onUpdateAvatar(avatarUrl, password);
        } catch (reason) { setAvatarError(reason instanceof Error ? reason.message : 'No se pudo sincronizar la foto.'); }
  ...
  const saveManagedDescription = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!selectedDescriptionUser || !managedDescription.trim() || descriptionSaving) return;
    const password = window.prompt('Ingresá la contraseña de Denis para guardar esta descripción.');
    if (!password) return;
    try {
      setDescriptionSaving(true); setDescriptionError(null);
      await onUpdateAiDescription(selectedDescriptionUser.id, managedDescription.trim(), password);
    } catch (reason) { setDescriptionError(reason instanceof Error ? reason.message : 'No se pudo guardar la descripción.'); }
  ...
  return <div className="space-y-4 pb-24 animate-fadeIn">
    <section className="relative overflow-hidden rounded-[26px] border border-white/15 bg-gradient-to-b from-white/[0.06] to-zinc-950/80 p-5 text-center backdrop-blur-xl">
      ...
      <h2 className="text-xl font-extrabold text-white">{currentUser.name}</h2>{isAdmin && <span className="mt-2 inline-flex items-center gap-1 rounded-full border border-[#0A84FF]/40 bg-[#0A84FF]/15 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-[#5AC8FA]"><ShieldCheck className="h-3.5 w-3.5" /> Administración</span>}<p className="mt-1 text-xs text-zinc-400">🎂 Cumpleaños: {currentUser.birthday} <span className="px-1">•</span> 📍 Puerto Madero</p>{avatarSaving && <p className="mt-3 text-xs text-[#5AC8FA]">Sincronizando foto...</p>}{avatarError && <p className="mt-3 text-xs text-red-300">{avatarError}</p>}
    </section>
    ...
    <section className="rounded-[26px] border border-[#0A84FF]/30 bg-[#0A84FF]/10 p-4"><div className="mb-2 flex items-center gap-1.5 text-xs font-bold text-[#5AC8FA]"><Sparkles className="h-4 w-4" /> Descripción IA</div><p className="rounded-xl border border-white/10 bg-black/50 p-3 text-xs italic leading-relaxed text-zinc-200">“{currentUser.aiDescription}”</p></section>
    {isAdmin && <section className="rounded-[26px] border border-[#0A84FF]/35 bg-white/[0.04] p-4"><div className="mb-3 flex items-center gap-1.5 text-xs font-bold text-[#5AC8FA]"><ShieldCheck className="h-4 w-4" /> Administrar descripciones IA</div><form onSubmit={saveManagedDescription} className="space-y-2.5"><label className="block text-[11px] font-semibold text-zinc-300" htmlFor="description-member">Perfil</label><select id="description-member" value={selectedDescriptionUser?.id || ''} onChange={(event) => { setSelectedDescriptionUserId(event.target.value); setDescriptionError(null); }} className="w-full rounded-xl border border-white/15 bg-black/60 px-3 py-2.5 text-sm text-white outline-none focus:border-[#0A84FF]">{users.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select><label className="block text-[11px] font-semibold text-zinc-300" htmlFor="description-ia">Descripción</label><textarea id="description-ia" value={managedDescription} onChange={(event) => setManagedDescription(event.target.value)} maxLength={280} rows={4} className="w-full resize-none rounded-xl border border-white/15 bg-black/60 p-3 text-xs leading-relaxed text-white outline-none focus:border-[#0A84FF]" />{descriptionError && <p role="alert" className="text-xs text-red-300">{descriptionError}</p>}<div className="flex items-center justify-between gap-3"><span className="text-[10px] text-zinc-500">{managedDescription.length}/280</span><button type="submit" disabled={descriptionSaving || !managedDescription.trim()} className="flex items-center gap-1.5 rounded-xl bg-[#0A84FF] px-3 py-2 text-xs font-bold text-white disabled:opacity-50">{descriptionSaving ? 'Guardando...' : <><Save className="h-3.5 w-3.5" /> Guardar descripción</>}</button></div></form></section>}
